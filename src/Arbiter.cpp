...
Arbiter::Arbiter() {
    ...
    this->sub_bh_pass = this->nh.subscribe("behavior/pass", 3, &Arbiter::cb_bh_pass, this);
...
void Arbiter::cb_bh_turn(const final_project::behavior::ConstPtr& msg){
    if(msg->active){
        this->behavior_queue.push(std::pair<int, final_project::behavior>(PRIORITY_TURN, *msg));
    }
    ROS_DEBUG("Arbiter: Turn(%s) Fw: %.1f Turn: %.1f", msg->active ? "on" : "off", msg->vel_fw, msg->vel_turn);
}

void Arbiter::cb_bh_pass(const final_project::behavior::ConstPtr& msg){
    if(msg->active){
        this->behavior_queue.push(std::pair<int, final_project::behavior>(PRIORITY_PASS, *msg));
    }
    ROS_DEBUG("Arbiter: Pass(%s) Fw: %.1f Turn: %.1f", msg->active ? "on" : "off", msg->vel_fw, msg->vel_turn);
}
